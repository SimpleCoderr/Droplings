package me.nodemoteplesk.droplings.util;
...
public class HoloAPI
{
    ...
    if (hasHolographicDisplays()) {
      try
      {
        if (hologramLocation != null)
        {
          ...
          return HolographicDisplaysAPI.createHologram(Droplings.getInstance(), hologramLocation, (String[])lines.toArray(new String[lines.size()]));
        }
      }
      catch (Exception ex) {}
    }
    ...
    if (hasHolographicDisplays()) {
      try
      {
        if (hologramLocation != null)
        {
          ...
          lines = MessageUtil.replace(lines);
          return HolographicDisplaysAPI.createIndividualHologram(Droplings.getInstance(), hologramLocation, Arrays.asList(new Player[] { player }), lines.isEmpty() ? new String[] { "" } : (String[])lines.toArray(new String[lines.size()]));
        }
      }
      catch (Exception ex) {}
    }
  ...
  public static Hologram[] getHolograms()
  {
    if (hasHolographicDisplays()) {
      try
      {
        return HolographicDisplaysAPI.getHolograms(Droplings.getInstance());
      }
      catch (Exception ex) {}
    }
    return null;
  ...
  {
    return (Bukkit.getServer().getPluginManager().isPluginEnabled("HolographicDisplays")) && (Droplings.getInstance().getConfig().getBoolean("holograms", true));
  }
}
